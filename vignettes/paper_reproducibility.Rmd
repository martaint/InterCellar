...
# Result #3: chemokine-related interactions are increased in critical and moderate, compared to control

```{r chemo, eval=FALSE}
ctrl_data <- read.csv("../../paper_reproducibility/res3/Gene-verse_dotplot_data_control.csv")
mod_data <- read.csv("../../paper_reproducibility/res3/Gene-verse_dotplot_data_MOD.csv")
cri_data <- read.csv("../../paper_reproducibility/res3/Gene-verse_dotplot_dataCRI.csv")

ctrl_data$pheno <- "Control"
mod_data$pheno <- "Moderate"
cri_data$pheno <- "Critical"

dotplot_data <- rbind(ctrl_data, mod_data, cri_data)

dotplot_data$pheno <- factor(dotplot_data$pheno, levels = c("Control", "Moderate", "Critical"))

# Subset to only secretory (plus diff)
dotplot_data_sub <- dotplot_data[grep("^Secretory", dotplot_data$cluster_pair),]



g <- ggplot(dotplot_data_sub, aes(x = int_pair, y = cluster_pair)) +
        geom_point(aes(color = score, size=3.5)) + 
        theme_minimal() +
        scale_color_gradient(low = "aquamarine", 
                             high = "#131780") + 
        facet_grid(groups_x ~ pheno, scales = "free", space = "free") +
        theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), 
              text = element_text(size=20),
...
tiff("../../paper_reproducibility/res3/chemokine_dotplot.tiff", height = 1000, width = 700)
plot(g)
dev.off()
```
